...
function Sidebar({ active, onOpenView, user, onLogout, onOpenUserMenu, theme, onToggleTheme, token, onVoiceInterpretation }) {
...
function AppContent() {
  ...
  return (
    <div style={{ display:'flex', width:'100%', minHeight:'100vh', background:T.bg, fontFamily:'var(--font-sans)',
      ...
      <Sidebar active={view} onOpenView={openRootView} user={user} onLogout={logout} onOpenUserMenu={() => setUserMenuOpen(true)} theme={theme} onToggleTheme={() => setTheme(prev => prev === 'dark' ? 'light' : 'dark')} token={token} onVoiceInterpretation={handleVoiceInterpretation} />
