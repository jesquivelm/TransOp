...
async function estimateRouteMetrics(origin, destination, options = {}) {
    const [from, to] = await Promise.all([
        resolveRoutePoint(origin, options.originCoords),
        resolveRoutePoint(destination, options.destinationCoords),
    ]);
...
app.post('/api/tms/usuarios', authenticateToken, requireAdmin, async (req, res) => {
    ...
    const normalizedRole = allowedRoles.has(rol) ? rol : 'operador';
...
app.delete('/api/tms/usuarios/:id', authenticateToken, requireAdmin, async (req, res) => {
    const { id } = req.params;

    try {
        const currentUser = await pgQuery(
            'SELECT id, es_root FROM usuarios WHERE id = $1',
            [id]
        );
        ...
        if (user.es_root) {
            return res.status(400).json({ error: 'No se puede desactivar el usuario root.' });
        }
...
app.delete('/api/tms/conductores/:id', authenticateToken, requireAdmin, async (req, res) => {
    const { id } = req.params;
    try {
        await ensureDatabaseCompatibility();
        await pgQuery(
...
app.delete('/api/tms/vehiculos/:id', authenticateToken, requireAdmin, async (req, res) => {
    const { id } = req.params;
    try {
        await ensureDatabaseCompatibility();
        await pgQuery(
...
app.post('/api/tms/routes/estimate-distance', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const metrics = await estimateRouteMetrics(origin, destination, { originCoords, destinationCoords });
        if (!metrics?.km) {
            return res.status(404).json({ success: false, error: 'No se pudo estimar la distancia con las ubicaciones indicadas.' });
        }

        res.json({ success: true, km: metrics.km, durationSeconds: metrics.durationSeconds });
    } catch (error) {
